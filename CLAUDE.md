...
- `petarin:settings` = `{ side, collapsedTranslucent, translucentOpacity, showOnPage, creatorRatio, font, fontSize, lineNumbers, defaultColor, syncEnabled, syncSettings, syncScope, syncDomains }`
  - `font` は `FONTS` の id（既定 `system`＝端末標準スタック・未知は system）。`fontSize` は px（既定 15・`FONT_SIZES` の離散値）。`lineNumbers` は編集時の行ガター表示（既定 false）。`defaultColor` は「最後に選んだ色」＝次の新規作成の初期色（`COLORS` の id・既定 `yellow`）。
