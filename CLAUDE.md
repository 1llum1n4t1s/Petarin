...
ドメイン単位の WEB ページ付箋 Chrome 拡張（MV3）＋ Android アプリ（`mobile/`・Capacitor）＋ iOS アプリ（`mobile_flutter/`・Flutter）＋ クラウド同期リレー（`infra/cloudflare/relay/`・Cloudflare Workers）。利用者向けの説明は [`README.md`](README.md)。
...
- `petarin:trash` = `TrashEntry[]`（単一集約リスト・ゴミ箱）
  - `TrashEntry = { domain, note: Note, deletedAt, origin: "user"|"sync" }`。一意キー = `(domain, note.id)`（同キーは `deletedAt` 新しい方）。**全ドメイン共通で最大 `TRASH_MAX`(=100) 件**（`deletedAt` 降順・超過は完全削除・TTL なし）。
