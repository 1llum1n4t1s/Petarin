...
トレイメニューは `ぺたりん v<version>`（押せない見出し）/ `更新を確認…` / `付箋デスク…` / `Windows 起動時に開く`（チェック）/ `ライセンス…` / `終了`。

- **同じメニューをレールの右クリックからも出す**（`show_rail_menu`）。Windows 11 はトレイアイコンを既定でオーバーフローに隠すので、画面に出ているレールから届く方が早い。メニューは**トレイと同一インスタンスを共有**する（更新項目のラベルが進捗表示を兼ねるため、複製すると片方だけ古い表示で残る）。**展開した付箋の中だけは既定の右クリックを残す**（本文のコピー/貼り付けを潰さない）。閉じ方はトレイのメニューと同じで、**どこかをクリックすると閉じる／Escape では閉じない**（レール窓のフォーカスの有無ではなくメニュー実装側の挙動。トレイでも同じで、`set_focus` しても変わらないことを実測済み）。
- **スタートアップ登録**は `tauri-plugin-autostart`（Windows では HKCU の `Run` キー）。自前で書くと exe パスの引用符・更新時のパス追従・解除漏れを全部抱えることになる。Velopack のインストール先は `…\Petarin\current\` で**更新してもパスが変わらない**ので登録し直しは要らない。チェックの表示は必ず**レジストリを読み直してから**合わせる（失敗したときに表示だけ ON になると「登録したのに起動しない」になる）。
