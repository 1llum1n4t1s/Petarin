...
//  - 同期先はブラウザ標準同期（chrome.storage.sync）だけ＝off / chrome の 2 モード。

import { STORAGE_KEYS, DEFAULT_SETTINGS, PROFILES_KEY, ensureProfiles, purgeLegacyCloudSync } from "../shared/storage.js";
...
chrome.runtime.onInstalled.addListener(async () => {
  ...
  // 旧クラウド同期を使っていた端末のペアリング鍵と syncMode を掃除する（撤去マイグレーション）。
  // 設定を書き換えるので、キャッシュ更新より先に走らせる。
  await purgeLegacyCloudSync().catch((e) => console.warn("[petarin] 旧同期設定の掃除に失敗:", e));
  const fixed = await chrome.storage.local.get(STORAGE_KEYS.settings);
  applySettingsCache(fixed[STORAGE_KEYS.settings]);
...
chrome.storage.onChanged.addListener((changes, area) => {
  ...
  if (area === "local") {
    // 設定変更は syncEnabled キャッシュを更新してから reconcile。
