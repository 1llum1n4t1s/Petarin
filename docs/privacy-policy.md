...
「付箋デスク」で同期をオンにし「ブラウザ標準同期」を選んだ場合に限り、同期対象として選んだプロファイル（付箋の保存単位）の付箋データが、お使いのブラウザに組み込まれた同期ストレージ（`chrome.storage.sync`）を通じて、**ブラウザのベンダーが提供する同期サービス**（Google アカウント／Microsoft アカウント／Mozilla アカウント等、利用者がブラウザにログインしているアカウント）経由で端末間に同期されます。
...
- 付箋の保存単位（プロファイル）を識別するキーも、端末側で**ハッシュ化（HMAC）**してから送ります。プロファイルの名前そのものは付箋本文と同じく暗号化して送るため、中継サーバーは**どんな名前の束に何が入っているか**も分かりません。
...
- **alarms**: クラウド同期をオンにしている間だけ、中継サーバーとの接続を定期的に張り直すために使用します（オフの間は使いません）。
